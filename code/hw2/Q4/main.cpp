...
void push_new_pair(deque<Node> &candy_lst, priority_queue<Pair *, vector<Pair *>, cmp> &pq, ll l, ll r)
{
    if (l != -1)
    {
        ...
        {
            cout << "push_new_pair error:: pushing invalid index pair to pq! : no right index\n";
            exit(1);
        }
    }
    else
    {
        cout << "push_new_pair error:: pushing invalid index pair to pq! : no left index\n";
        exit(1);
    }
}

void print_ans(vector<ll> &ans_arr, ll N)
{
    for (ll i = 1; i <= N; i++)
    {
...
ll clean_input(deque<Node> &candy_lst, ll *raw_candy, long long size)
{
    bool cont_pos = 0, cont_neg = 0;

    // first input one element
    if (raw_candy[0] < 0)
    {
        cont_neg = 1;
    }
    ...
    for (long long i = 1; i < size; i++)
    {
        ...
        if (raw_candy[i] < 0)
        {
            ...
            if (cont_neg)
            {
                if (candy_lst.back().val < raw_candy[i])
                    candy_lst[candy_lst.size() - 1].val = raw_candy[i];
            }
            else
            {
                candy_lst.emplace_back(raw_candy[i]);
                cont_neg = 1;
            }
        }
        // candy>0
...
int main()
{
    ...
    while (T--)
    {
        ...
        // preprocessing input
        ans_arr[1] = clean_input(candy_lst_big, raw_candy, N);
        //TODO  modify k, pair_arr should refer to k not N (check)
        k_big = candy_lst_big.size();

        //TODO  modify ans_arr to k k-1 (check)
        for (ll i = 0; i < k_big; i++)
        {
            total_big += candy_lst_big[i].val;
        }
